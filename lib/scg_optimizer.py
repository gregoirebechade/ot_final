...
def scg(a, b, M, reg1, reg2, reg3, beta, f1, f2, f3, df1, df2, df3, j_dist, G0=None, numItermax=10, numInnerItermax=50, 
    ...
    coef_lw = 1/10 # gregoire
    coef_gw = 1/10 # gregoire 



    beta*=coef_lw # gregoire
    reg1*=coef_gw # gregoire
    reg2*=coef_lw # gregoire
    reg3*=coef_lw # gregoire
    lambda_g*=coef_gw # gregoire
    ...
    def cost(G):
        return np.sum(M * G) + reg2 * f2(G) + reg3 * f3(G) + beta * np.linalg.norm(G,'fro')**2 + reg1 * f1(G) - lambda_g * (np.sum(G * np.log(G)) - np.sum(G * np.log(j_dist))) # original 
        # return np.sum(M * G) + reg2 * f2(G) + reg3 * f3(G) + beta * np.linalg.norm(G,'fro')**2  - 1e-4 * (np.sum(G * np.log(G)) - np.sum(G * np.log(j_dist))) # greg 
